...
test.describe("Dashboard", () => {
    ...
    test("renders Quick settings and empty recent state", async ({ cleanPage: page }) => {
        ...
        await expect(page.getByText("No documents found. Start by creating one!")).toBeVisible();
        await expect(page.getByText("Mark notes as favorites to see them here.")).toBeVisible();
    });

    test("New Document quick action opens the create note dialog", async ({ cleanPage: page }) => {
