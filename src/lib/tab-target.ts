...
export type TabTargetType = "note" | "ai-chat";
...
export function isNoteTab(tab: { type?: string | null }): boolean {
  return !isAiChatTab(tab);
}

export function isNoteTabForNoteId(
  tab: Pick<TabTargetCandidate, "noteId" | "type">,
  noteId: string,
): boolean {
...
export function getTabTargetType(tab: { type?: string | null }): TabTargetType {
  return isAiChatTab(tab) ? "ai-chat" : "note";
}

export function getTabTarget(
  tab: Pick<TabTargetCandidate, "noteId" | "type">,
): TabTarget {
...
export function getTabTargetKeyForTab(
  tab: Pick<TabTargetCandidate, "noteId" | "type">,
): string {
...
export function getRouteForTab(
  tab: Pick<TabTargetCandidate, "noteId" | "type">,
): string {
  return getRouteForTabTarget(getTabTarget(tab));
}

export function getCurrentRouteTabTarget(
  pathname: string,
  search: string,
): TabTarget | null {
...
export function findTabByTarget<T extends TabTargetCandidate>(
  tabs: T[],
