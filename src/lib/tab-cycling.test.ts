...
import { getCycledTabNoteId, getCycledTabTarget, getTabTargetAtIndex } from "./tab-cycling";
...
const mixedTabs = [
    ...
    { id: "note-tab-2", noteId: "note-2", order: 3, type: "tab" },
];

describe("getCycledTabTarget", () => {
    it("cycles forward across note, AI chat, and terminal tabs", () => {
        expect(
            getCycledTabTarget({
                tabs: mixedTabs,
        ...
        expect(
            getCycledTabTarget({
                tabs: mixedTabs,
                currentTarget: { type: "terminal", id: "terminal-1" },
                activeTabId: null,
                direction: "forward",
            }),
    ...
    it("cycles backward across note, AI chat, and terminal tabs", () => {
        expect(
            getCycledTabTarget({
                tabs: mixedTabs,
...
describe("getTabTargetAtIndex", () => {
    ...
    it("selects the last ordered tab", () => {
        expect(getTabTargetAtIndex({ tabs: mixedTabs, index: "last" })).toStrictEqual({
            type: "note",
            id: "note-2",
        });
    });
