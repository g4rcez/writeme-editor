...
export function generateNotePath(rootDir: string, noteTitle: string): string {
    ...
    const sanitizedSegments = segments.map((s) => sanitizeFilename(s)).filter(Boolean);
 ...
 */
export function getUniqueNoteTitle(baseTitle: string, existingNotes: { title: string }[]): string {
...
export const getReadingTime = (text: string, wordsPerMinute: number = 225): ReadingTimeResult => {
    const words = text
        .trim()
        .split(/\s+/)
