...
function selectItemKey(item: SelectItem, fromTable: string, queryTables: Set<string>): string {
    if (item.type === "Column") {
...
function computeAggregate(
    ...
    if (fn === "COUNT") {
        ...
        return groupRows.filter((r) => resolveField(r, column, fromTable, queryTables) != null).length;
    }
...
function applyGroupBy(rows: Row[], query: Query, fromTable: string, queryTables: Set<string>): Row[] {
    ...
    for (const row of rows) {
        const key = groupByCols
            .map((col) => String(resolveField(row, col, fromTable, queryTables) ?? "NULL"))
            .join("|||");
    ...
    return Array.from(groups.values()).map((groupRows) => {
        ...
        if (selectItems.length > 0) {
            for (const item of selectItems) {
                ...
                if (item.type === "Column") {
                    resultRow[key] = resolveField(groupRows[0]!, item, fromTable, queryTables);
                } else {
                    resultRow[key] = computeAggregate(item.fn, item.column, groupRows, fromTable, queryTables);
                }
            ...
            for (const col of groupByCols) {
                ...
                resultRow[key] = resolveField(groupRows[0]!, col, fromTable, queryTables);
            }
...
function resolveOrderByValue(row: Row, column: ColumnRef, fromTable: string, queryTables: Set<string>): unknown {
