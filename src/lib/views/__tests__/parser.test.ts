...
describe("parser", () => {
    ...
    it("parses multiple JOINs", () => {
        const q = parse("FROM notes JOIN hashtags ON notes.id = hashtags.filename JOIN tabs ON notes.id = tabs.noteId");
    ...
    it("parses SELECT with COUNT(*)", () => {
        const q = parse('SELECT noteType, COUNT(*) as "Total" FROM notes GROUP BY noteType');
        expect(q.select!.columns).toHaveLength(2);
        expect(q.select!.columns[0]).toMatchObject({
            type: "Column",
            path: ["noteType"],
    ...
    it("parses SELECT with SUM/AVG/MIN/MAX", () => {
        const q = parse("SELECT SUM(fileSize), AVG(fileSize), MIN(fileSize), MAX(fileSize) FROM notes");
