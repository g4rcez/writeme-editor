...
describe("engine", () => {
    ...
    it("filters by tags CONTAINS", () => {
        const result = executeQuery(parse("WHERE tags CONTAINS 'book'"), dataSources);
        expect(result).toHaveLength(3);
    });

    it("filters by content CONTAINS", () => {
        const result = executeQuery(parse("WHERE content CONTAINS 'ring'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("4");
    });

    it("filters by metadata field", () => {
        const result = executeQuery(parse("WHERE metadata.author = 'Herbert'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("1");
    });

    it("filters by metadata numeric comparison", () => {
        const result = executeQuery(parse("WHERE metadata.rating >= 4"), dataSources);
        expect(result).toHaveLength(3);
    });

    it("filters with AND", () => {
        const result = executeQuery(parse("WHERE tags CONTAINS 'book' AND metadata.rating >= 5"), dataSources);
        expect(result).toHaveLength(2);
    });

    it("filters with OR", () => {
        const result = executeQuery(parse("WHERE noteType = 'quick' OR noteType = 'template'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("3");
    });

    it("filters with NOT", () => {
        const result = executeQuery(parse("WHERE NOT favorite = true"), dataSources);
        expect(result).toHaveLength(4);
    });
    ...
    it("filters by STARTS_WITH", () => {
        const result = executeQuery(parse("WHERE title STARTS_WITH 'Meet'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("5");
    });

    it("filters by LIKE pattern", () => {
        const result = executeQuery(parse("WHERE title LIKE '%notes%'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("5");
    });
    ...
    it("sorts DESC by metadata.rating", () => {
        const result = executeQuery(parse("WHERE tags CONTAINS 'book' ORDER BY metadata.rating DESC"), dataSources);
        expect((result[0]!["notes.metadata"] as Record<string, unknown>).rating).toBe(5);
        expect((result[result.length - 1]!["notes.metadata"] as Record<string, unknown>).rating).toBe(4);
    });
    ...
    it("filters by date comparison", () => {
        const result = executeQuery(parse("WHERE createdAt < '2024-01-01'"), dataSources);
        expect(result).toHaveLength(1);
        expect(result[0]!["notes.id"]).toBe("5");
    });
    ...
    it("handles complex query", () => {
        const result = executeQuery(
            parse("WHERE tags CONTAINS 'book' AND metadata.author != 'Martin' ORDER BY metadata.rating DESC LIMIT 2"),
            dataSources,
        );
        expect(result).toHaveLength(2);
        result.forEach((row) => {
            expect(result[0]!["notes.tags"]).toContain("book");
            expect((row["notes.metadata"] as Record<string, unknown>).author).not.toBe("Martin");
        });
    });
});
...
describe("engine - JOIN", () => {
    ...
    it("joins notes with hashtags", () => {
        const result = executeQuery(parse("FROM notes JOIN hashtags ON notes.id = hashtags.filename"), joinSources);
        expect(result).toHaveLength(2);
    ...
    it("returns empty when no rows match the join", () => {
        const result = executeQuery(parse("FROM notes JOIN hashtags ON notes.id = hashtags.filename"), {
            notes,
            hashtags: [
                {
                    id: "h1",
    ...
    it("supports non-notes primary table", () => {
        ...
        const result = executeQuery(parse("FROM projects JOIN tabs ON projects.id = tabs.project"), { projects, tabs });
        expect(result).toHaveLength(2);
...
describe("engine - GROUP BY", () => {
    ...
    it("groups by noteType with COUNT(*)", () => {
        const result = executeQuery(
            parse('SELECT noteType, COUNT(*) as "Total" FROM notes GROUP BY noteType'),
            groupSources,
        );
        expect(result).toHaveLength(2);
    ...
    it("groups with SUM aggregate", () => {
        const result = executeQuery(parse("SELECT noteType, SUM(fileSize) FROM notes GROUP BY noteType"), groupSources);
        const noteRow = result.find((r) => r["notes.noteType"] === "note");
        const quickRow = result.find((r) => r["notes.noteType"] === "quick");
    ...
    it("groups with AVG aggregate", () => {
        const result = executeQuery(parse("SELECT noteType, AVG(fileSize) FROM notes GROUP BY noteType"), groupSources);
        const noteRow = result.find((r) => r["notes.noteType"] === "note");
    ...
    it("groups with MIN and MAX aggregates", () => {
        const result = executeQuery(
            parse("SELECT noteType, MIN(fileSize), MAX(fileSize) FROM notes GROUP BY noteType"),
            groupSources,
        );
        const noteRow = result.find((r) => r["notes.noteType"] === "note");
    ...
    it("groups with WHERE filter applied before grouping", () => {
        const result = executeQuery(
            parse('SELECT noteType, COUNT(*) as "Total" FROM notes WHERE favorite = true GROUP BY noteType'),
            groupSources,
        );
        expect(result).toHaveLength(2);
        const noteRow = result.find((r) => r["notes.noteType"] === "note");
    ...
    it("supports ORDER BY on alias after GROUP BY", () => {
        const result = executeQuery(
            parse("SELECT noteType, COUNT(*) as total FROM notes GROUP BY noteType ORDER BY total DESC"),
            groupSources,
        );
    ...
    it("handles aggregate without GROUP BY (implicit single group)", () => {
        const result = executeQuery(parse("SELECT COUNT(*) FROM notes"), groupSources);
        expect(result).toHaveLength(1);
    ...
    it("GROUP BY without SELECT includes group columns and COUNT(*)", () => {
        const result = executeQuery(parse("FROM notes GROUP BY noteType"), groupSources);
        expect(result).toHaveLength(2);
        result.forEach((row) => {
