...
export function findCompatibleNote(
  notes: Note[],
  target: CompatibleNoteTarget | string,
): Note | undefined {
  const rawValues = getRawTargetValues(target);
  const hasQualifiedTarget = rawValues.some((value) =>
    stripSubpath(value).includes("/"),
  );
...
export function createLookupValues(
  target: CompatibleNoteTarget | string,
): Set<string> {
...
function findByLookupValues(
  ...
  return notes.find((note) => {
    ...
    return candidates.some((candidate) =>
      lookupValues.has(normalizeLookupValue(candidate)),
    );
  });
}
...
function createNormalizedValueSet(
  values: string[],
  { includeBasename }: { includeBasename: boolean },
): Set<string> {
  ...
  values.forEach((value) => {
    ...
    if (includeBasename) {
      variants.push(
        basename(decodedValue),
        removeMarkdownExtension(basename(decodedValue)),
      );
    }
