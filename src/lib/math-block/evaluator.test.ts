import { __resetMathInstanceCache, classifyLine, evaluateMathBlock, getMathInstance, type MathLine } from "solver";
...
describe("evaluateMathBlock", () => {
    ...
    it("10k + 2M evaluates to 2,010,000 (formatted by mathjs notation:auto)", () => {
        ...
        const val = (result[0] as { result: { ok: true; value: string } }).result.value;
    ...
    it("calculates days-to questions in math blocks", () => {
        ...
        const result = evaluateMathBlock(lines("days to 15/06/2026", "how many days to 19/06/2026"));
    ...
    it("error on one line does not corrupt scope", () => {
        const result = evaluateMathBlock(lines("x = 10", "undefinedVar + 1", "x + 5"));
    ...
    it("CSS unit px to em conversion with default 16px em", () => {
        ...
        const value = (result[0] as { result: { ok: true; value: string } }).result.value;
    ...
    it("em = N overrides CSS em unit for subsequent lines", () => {
        ...
        const value = (result[1] as { result: { ok: true; value: string } }).result.value;
        expect(value).toContain("2");
    });
    ...
    it("multi-word label is not injected into scope", () => {
        const result = evaluateMathBlock(lines("total revenue: 500", "total revenue + 1"));
