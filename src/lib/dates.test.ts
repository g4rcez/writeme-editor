...
describe("Dates.evaluateDateArithmetic", () => {
  ...
  it("today + N minutes (same day)", () => {
    expect(Dates.evaluateDateArithmetic("today + 60 minutes")).toBe(
      "2025-01-15",
    );
  });

  it("today + N seconds (crosses midnight)", () => {
    expect(Dates.evaluateDateArithmetic("today + 86400 seconds")).toBe(
      "2025-01-16",
    );
  });
  ...
  it("today + 2 decades", () => {
    expect(Dates.evaluateDateArithmetic("today + 2 decades")).toBe(
      "2045-01-15",
    );
  });
  ...
  it("chained operations", () => {
    expect(Dates.evaluateDateArithmetic("today + 1 month + 2 days")).toBe(
      "2025-02-17",
    );
  });
  ...
  it("anchored chain with mixed signs", () => {
    expect(Dates.evaluateDateArithmetic("today + 5 days - 2 days")).toBe(
      "2025-01-18",
    );
  });

  it("anchored chain mixing units", () => {
    expect(Dates.evaluateDateArithmetic("today + 1 week - 3 days")).toBe(
      "2025-01-19",
    );
  });
  ...
  it("relative chain with mixed unit ops", () => {
    expect(Dates.evaluateDateArithmetic("2 weeks + 3 days - 1 day")).toBe(
      "2025-01-31",
    );
  });
...
describe("Dates.evaluateNatural", () => {
  ...
  it("dispatches to evaluateEpoch", () => {
    expect(Dates.evaluateNatural("epoch 1234567890")).toBe(
      "2009-02-13 23:31:30 UTC",
    );
  });
