...
function filterMigratableTabs(tabs: unknown[]): unknown[] {
	return tabs.filter((tab): boolean => {
		if (!tab || typeof tab !== "object") return true;
		return (tab as MaybeTab).type !== TERMINAL_TAB_TYPE;
	});
}

async function importAllData(data: MigrationPayload): Promise<MigrationCounts> {
	const tabs = filterMigratableTabs(data.tabs);
	await Promise.all([
		db.notes.bulkPut(data.notes as Parameters<typeof db.notes.bulkPut>[0]),
		db.tabs.bulkPut(tabs as Parameters<typeof db.tabs.bulkPut>[0]),
		db.hashtags.bulkPut(
...
export async function exportToFile(): Promise<void> {
	...
	const payload: MigrationPayload = {
		...
		version: 1,
		notes,
		tabs: filterMigratableTabs(tabs),
		hashtags,
		settings,
		scripts,
	};
