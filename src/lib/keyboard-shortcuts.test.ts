...
describe("keyboard shortcuts", () => {
	...
	it("matches Ctrl/Cmd+1-8 as indexed tab navigation", () => {
		expect(
			getTabNavigationShortcut(event({ ctrlKey: true, key: "1" })),
		).toStrictEqual({
			type: "index",
			index: 0,
		});
		expect(
			getTabNavigationShortcut(event({ metaKey: true, key: "8" })),
		).toStrictEqual({
			type: "index",
			index: 7,
		});
	...
	it("matches Ctrl/Cmd+9 as last tab navigation", () => {
		expect(
			getTabNavigationShortcut(event({ ctrlKey: true, key: "9" })),
		).toStrictEqual({
			type: "last",
		});
		expect(
			getTabNavigationShortcut(event({ metaKey: true, key: "9" })),
		).toStrictEqual({
			type: "last",
		});
	});
	...
	it("rejects tab number shortcuts with unsupported modifiers or invalid numbers", () => {
		...
		expect(
			getTabNavigationShortcut(
				event({ ctrlKey: true, shiftKey: true, key: "1" }),
			),
		).toBeNull();
		expect(
			getTabNavigationShortcut(
				event({ ctrlKey: true, altKey: true, key: "1" }),
			),
		).toBeNull();
		expect(
			getTabNavigationShortcut(event({ ctrlKey: true, key: "0" })),
		).toBeNull();
		expect(
			getTabNavigationShortcut(event({ ctrlKey: true, key: "n" })),
		).toBeNull();
	});
});
