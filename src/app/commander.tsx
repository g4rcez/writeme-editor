...
export const Commander = (props: Props) => {
    ...
    const openedTabsGroup = useMemo(
        (): CommandItemTypes[] =>
            [...props.tabs]
                ...
                .map((tab): CommandItemTypes => {
                    ...
                    const terminalSession = isTerminal ? terminalSessionsById.get(tab.noteId) : undefined;
        ...
        [props.tabs, notesById, chatsById, terminalSessionsById, dispatch, navigate],
    );
    ...
    const getTerminalTabTitle = useCallback(
        (tab: Tab): string => terminalSessionsById.get(tab.noteId)?.title?.trim() || "Terminal",
            ...
            items: [
                    ...
                    ? [
                          {
                              ...
                              action: async (args: { setOpen: (v: boolean) => void }) => {
                                  ...
                                  try {
                                      ...
                                      const notes = props.notes.some((n) => n.id === updatedNote.id)
                                          ? props.notes.map((n) => (n.id === updatedNote.id ? updatedNote : n))
                                          : props.notes.concat(updatedNote);
                                      dispatch.setNote(updatedNote);
                                      dispatch.notes(notes);
                                      notificationRef.current?.(<span>Added {updatedNote.title} to favorites</span>, {
                                          theme: "success",
                                          closable: true,
                                          timeout: 3000,
                                      });
                                      ...
                                      uiDispatch.setError(
                                          error instanceof Error ? error.message : "Failed to add note to favorites",
                                      );
                                  }
                              },
                          },
                    ...
                    : []),
                {
                    ...
                    action: (args) => {
                        ...
                        const content = editor ? getEditorMarkdown(editor) : (props.note?.content ?? "");
        ...
        const otherStuff: CommandItemTypes[] = [
            {
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        action: (args) => {
                            args.setOpen(false);
                            closeTabsSequentially([...props.tabs].sort((a, b) => a.order - b.order));
                        },
                    },
                    {
                        type: "shortcut",
                  ...
                  items: [
                      {
                          ...
                          action: async (args) => {
                              ...
                              if (result.success) {
                                  notificationRef.current?.(<span>CLI installed at {result.installPath}</span>, {
                                      theme: "success",
                                      closable: true,
                                      timeout: 4000,
                                  });
                              } else {
                                  notificationRef.current?.(<span>Failed to install CLI: {result.error}</span>, {
                                      theme: "danger",
                                      closable: true,
                                      timeout: 6000,
                                  });
