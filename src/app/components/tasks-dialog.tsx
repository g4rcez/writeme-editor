...
import { setTaskItemChecked } from "@/app/elements/task-list-item";
...
export function parseEditorTasks(doc: ProseMirrorNode): Stack[] {
    ...
    let currentTaskListPos: number | undefined;
            ...
            if (isInTaskList) {
                ...
                if (firstNode && firstNode.type.name === "taskItem") {
                    ...
                    if (slice.content.childCount > 1) {
                        tr.insert(mappedInsertPos + (taskList?.nodeSize ?? 0), slice.content.cut(firstNode.nodeSize));
                    }
                } else {
                    tr.insert(mappedInsertPos, slice.content);
                }
        ...
        .command(({ tr, state }) => {
            ...
            if (updates.description !== undefined) {
                if (card.descriptionPos !== undefined) {
                    ...
                    if (descriptionNode) tr.insert(tr.mapping.map(insertPos), descriptionNode);
                }
            }
            return true;
        })
...
const NewTaskModal = ({ isOpen, onClose, onAdd, stackTitle }: NewTaskModalProps) => {
    ...
    return (
        <Modal open={isOpen} onChange={onClose} title={`Add task to "${stackTitle}"`} className="max-w-md">
                ...
                <div className="flex-1 min-w-0" onPointerDown={(e) => e.stopPropagation()}>
                    {editingField === "title" ? (
                        ...
                        <span
                            ...
                            className={`text-sm font-medium leading-snug break-words cursor-text hover:bg-muted rounded px-1 -mx-1 transition-colors ${
...
function DroppableColumn({ stack, onAddTask, children }: DroppableColumnProps) {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`w-80 flex-shrink-0 flex flex-col gap-3 rounded-xl bg-muted/50 p-4 transition-[box-shadow,background-color] duration-300 ${
                isOver ? "ring-2 ring-primary/40 bg-primary/5 shadow-inner" : "ring-1 ring-transparent"
            }`}
        >
            <div className="flex items-center justify-between px-1 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-bold transition-colors ${
                            isOver ? "bg-primary text-primary-foreground" : "bg-muted text-foreground/70"
                        }`}
                    >
                        {stack.cards.length}
                    </span>
                ...
                <button
                    ...
                    className="p-1 rounded-md hover:bg-muted text-foreground/70 transition-colors"
                    title="Add task"
                >
...
export const TasksDialog = () => {
    ...
    const toggleTask = (card: Card) => {
        ...
        editor
            .chain()
            .command(({ tr }) => {
                return setTaskItemChecked(tr, { taskPos: pos, checked: newChecked });
            })
            .run();
    };
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const finalLocInUI = { ...activeLoc };
        const targetStackInUI = { ...stacks[activeLoc.si]! };
                ...
                <AnimatePresence propagate>
                    {stacks.length === 0 ? (
                        <p className="py-8 text-sm text-center text-disabled">
                            No tasks found. Add headings or <code className="rounded bg-muted px-1">{"[ ]"}</code>{" "}
                        ...
                        >
                            ...
                            >
                                {activeId && getActiveCard() ? <TaskCard card={getActiveCard()!} isOverlay /> : null}
                            </DragOverlay>
                        </DndContext>
                    )}
                </AnimatePresence>
