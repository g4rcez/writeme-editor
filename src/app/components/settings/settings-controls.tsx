...
export function AppearanceSettingsControls({
  ...
  return (
    <SettingsField
      label="Theme"
      description="Choose the visual theme used across the editor."
      compact={compact}
    >
      <Select
        optionalText=" "
...
export function TrashSettingsControls({
  settings,
  onPatch,
  compact,
}: SettingsControlsProps) {
...
export function ShortcutSettingsControls({
  settings,
  onPatch,
  compact,
}: SettingsControlsProps) {
  ...
  return (
    <div className={compact ? "space-y-4" : "contents"}>
      <SettingsField
        label="Quick Note"
        description="Open a new quick note from anywhere."
        compact={compact}
      >
        <ShortcutRecorder
      ...
      >
        <Checkbox
          ...
          onChange={(event: ChangeEvent<HTMLInputElement>) =>
            onPatch({ autosave: event.target.checked })
          }
        />
      </SettingsField>
      {settings.autosave ? (
...
export function QuickSettingsControls({
  settings,
  onPatch,
  compact = false,
}: SettingsControlsProps) {
  return (
    <div className={compact ? "space-y-5" : "divide-y-0"}>
      <AppearanceSettingsControls
        ...
        compact={compact}
      />
      <TrashSettingsControls
        settings={settings}
        onPatch={onPatch}
        compact={compact}
      />
      <ShortcutSettingsControls
        settings={settings}
        onPatch={onPatch}
        compact={compact}
      />
