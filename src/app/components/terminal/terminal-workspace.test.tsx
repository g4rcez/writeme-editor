...
describe("TerminalWorkspace", () => {
    ...
    it("renders every open terminal panel so inactive sessions stay mounted", () => {
        render(
            <TerminalWorkspace
                ...
                terminalSessions={[createTerminalSession("terminal-1"), createTerminalSession("terminal-2")]}
        ...
        expect(screen.getByTestId("terminal-panel-terminal-1")).toHaveAttribute("data-active", "true");
        expect(screen.getByTestId("terminal-panel-terminal-2")).toHaveAttribute("data-active", "false");
        expect(screen.getByTestId("terminal-panel-terminal-1")).toHaveAttribute("data-cwd", "/workspace");
        expect(screen.getByTestId("terminal-panel-terminal-2")).toHaveAttribute("data-restart-notice", "true");
    });
    ...
    it("repairs missing terminal session metadata for restored terminal tabs", async () => {
        ...
        await waitFor(() => {
            expect(dispatch.ensureTerminalSession).toHaveBeenCalledWith("missing-terminal");
        });
    });
});
