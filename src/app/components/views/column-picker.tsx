...
export function ColumnPicker({ columns, onChange, metadataKeys = [], joinedTables = [] }: ColumnPickerProps) {
    ...
    return (
        <Dropdown
            buttonProps={{
                className:
                    "flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-foreground/70 transition-colors hover:bg-card-hover",
            }}
            trigger={
                <>
                    <ColumnsIcon size={14} />
        ...
        >
            <div className="flex flex-col gap-1 p-2 min-w-40 max-h-64 overflow-y-auto">
                {allCols.map((col) => (
                    ...
                    >
                        <Checkbox checked={isEnabled(col.field)} onChange={() => toggle(col)} />
