...
const okStatus = (overrides: Partial<Extract<GitStatusResult, { kind: "ok" }>> = {}): GitStatusResult => ({
    kind: "ok",
    branch: "main",
...
describe("GitSyncDialog", () => {
    ...
    it("enables Commit & push when message is typed and pushes successfully", async () => {
        ...
        window.electronAPI.git.commitAndPush = vi.fn().mockResolvedValue(pushResult);
        ...
        expect(window.electronAPI.git.commitAndPush).toHaveBeenCalledWith("/tmp/notes", "my commit");
        expect(closeGitDialog).toHaveBeenCalled();
        expect((notificationRef.current as ReturnType<typeof vi.fn>).mock.calls[0]![1]).toMatchObject({
            theme: "success",
        });
    });
    ...
    it("submits via Cmd/Ctrl+Enter in the textarea", async () => {
        ...
        expect(window.electronAPI.git.commitAndPush).toHaveBeenCalledWith("/tmp/notes", "shortcut commit");
        expect(closeGitDialog).toHaveBeenCalled();
    });
    ...
    it("renders not-a-repo state with Close button and no Commit button", async () => {
        ...
        await waitFor(() => screen.getByText("Not a git repository", { selector: "p.font-medium" }));
