...
describe("TreeView", () => {
  ...
  beforeEach(() => {
    ...
    statFile = vi.fn(async (path: string) => ({
      success: true,
      exists: Object.values(directoryEntries).some((entries) =>
    ...
    moveFile = vi.fn(async (oldPath: string, newPath: string) => {
      ...
      const moved = directoryEntries[oldParent]?.find(
        (entry) => entry.path === oldPath,
      );
      if (!moved) return { success: false, error: "missing source" };
      directoryEntries[oldParent] = directoryEntries[oldParent]!.filter(
        (entry) => entry.path !== oldPath,
      );
