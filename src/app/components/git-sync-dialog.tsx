...
export const GitSyncDialog = () => {
    ...
    return (
        <Modal open={open} onChange={(val) => (val ? uiActions.openGitDialog() : handleClose())} title="Git sync">
            <div className="flex flex-col gap-4 min-w-[28rem]">
                ...
                {state.status === "idle" || state.status === "loading" ? (
                    <p className="text-sm text-foreground/50">Reading repository status…</p>
                ) : null}

                {state.status === "git-missing" ? (
                    <div className="flex flex-col gap-2 text-sm">
                        <p className="font-medium">git not found</p>
                        <p className="text-foreground/50">Install git and make sure it is on your PATH.</p>
                ...
                {state.status === "not-a-repo" ? (
                    <div className="flex flex-col gap-2 text-sm">
                        ...
                        <p className="text-foreground/50">
                            Initialise the folder with <code>git init</code> and add a remote, then try again.
                        </p>
                        <div className="flex justify-end">
                            <Button type="button" theme="ghost-muted" onClick={handleClose}>
                                Close
                    ...
                    <Fragment>
                        <div className="flex items-baseline justify-between gap-4 text-sm">
                            <span>
                                ...
                                {state.upstream ? (
                                    <span className="text-foreground/50"> → {state.upstream}</span>
                                ) : (
                                    <span className="text-foreground/50"> (no upstream)</span>
                                )}
                            </span>
                            <span className="text-foreground/50" aria-label={countsAriaLabel(state.counts)}>
                        ...
                        <div className="flex justify-end gap-2">
                            {isElectron() ? (
                                <Button
                                    ...
                                    disabled={state.status === "pushing" || !state.hasChanges || generating}
                                >
                            ...
                            <Button
                                ...
                                onClick={submit}
                                disabled={
                                    state.status === "pushing" || !state.hasChanges || message.trim().length === 0
                                }
                            >
