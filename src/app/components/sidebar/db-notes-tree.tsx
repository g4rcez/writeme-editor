...
function buildVirtualTree(notes: Note[], rootPath: string): VirtualNode[] {
    ...
    const relevant = notes.filter((n) => n.filePath && n.filePath.startsWith(prefix));
...
function flattenVisible(nodes: VirtualNode[], expandedPaths: Set<string>, depth = 0): FlatNode[] {
...
export const DbNotesTree = ({ notes, rootPath }: DbNotesTreeProps) => {
    ...
    const tree = useMemo(() => buildVirtualTree(notes, rootPath), [notes, rootPath]);

    const flatNodes = useMemo(() => flattenVisible(tree, expandedPaths), [tree, expandedPaths]);
    ...
    if (flatNodes.length === 0) {
        return (
            <div className="p-4 text-xs text-center text-muted-foreground">No notes with files in this directory</div>
        );
    }
                ...
                if (node.type === "directory") {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className="text-sm truncate flex-1 text-foreground">{node.name}</span>
                        </div>
                    );
                }
