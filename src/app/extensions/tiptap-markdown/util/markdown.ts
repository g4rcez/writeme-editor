...
function scanDelims(text: string, pos: number): { can_open: boolean; can_close: boolean } {
...
function trimStart(text: string, delim: string, from: number, to: number): { text: string; from: number; to: number } {
    let pos = from,
        res = text;
    while (pos < to) {
...
function trimEnd(text: string, delim: string, from: number, to: number): { text: string; from: number; to: number } {
    let pos = to,
        res = text;
    while (pos > from) {
...
export function trimInline(text: string, delim: string, from: number, to: number): string {
    let state = {
        text,
        from,
        to,
