...
describe("link paste", () => {
    it("applies a pasted URL to selected text without replacing the label", () => {
        ...
        expect(applyPastedUrlToSelection(editor, "https://example.com/docs")).toBe(true);
        expect(editor.getMarkdown()).toBe("[selected text](https://example.com/docs)");
    ...
    it("keeps explicitly labelled markdown links as text links", () => {
        ...
        expect(editor.getMarkdown()).toBe("[Project docs](https://example.com/docs)");
        expect(editor.getHTML()).toContain('data-link-url="https://example.com/docs"');
    ...
    it("renders labelled links with a full URL hover trigger", async () => {
        ...
        await waitFor(() => {
            const link = container.querySelector<HTMLAnchorElement>('a[data-link-url="https://example.com/docs"]');
