...
type DomainLinkDisplayProps<As extends React.ElementType = React.ElementType<"a">> = { url: string; as?: As };
...
export function DomainLinkDisplay<As extends React.ElementType>({ url, as }: DomainLinkDisplayProps<As>) {
    const As = as || "a";
    const matched = getDomainConfigMatch(url);
    ...
    return (
        <As href={url} target="_blank" contentEditable={false} className={commonClasses} rel="noopener noreferrer">
            {displayContent}
        </As>
    );
}
...
const shouldRenderAsDomainLink = (element: HTMLElement, href: string): boolean => {
...
const DomainLinkView = (props: NodeViewProps) => {
    ...
    return (
        <NodeViewWrapper as="span" className="inline-block align-middle mx-1" data-link-url={href}>
...
export const DomainLink = Node.create({
    ...
    addStorage() {
        return {
            markdown: {
                serialize(state: MarkdownSerializerState, node: MarkdownSerializerNode) {
