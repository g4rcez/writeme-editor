...
export const RootLayout = () => {
  ...
  useEffect(() => {
    ...
    const activeTab = state.tabs.find(
      (tab) =>
        tab.id === state.activeTabId || (!isAiChatTab(tab) && !isTerminalTab(tab) && tab.noteId === state.activeTabId),
    );
    if (activeTab) {
  ...
  const cycleEditorTab = useCallback(
    ...
    [location.pathname, location.search, navigateToTab, state.activeTabId, state.tabs],
  );
