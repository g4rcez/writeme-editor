...
const { streamTextMock, stepCountIsMock, openAIChatMock, openAIResponsesMock } =
	vi.hoisted(() => ({
...
describe("createCodexRequestBody", () => {
	...
	it("adds fallback instructions when the SDK omits them", () => {
		...
		expect(JSON.parse(body)).toEqual(
			expect.objectContaining({ instructions: "Fallback instructions" }),
		);
	});
});
...
describe("OpenAIAdapter", () => {
	it("uses OpenAI chat completions for platform API keys", async () => {
		...
		for await (const event of adapter.sendMessage(
			[{ role: "user", content: { text: "List my notes" } }],
			{
