...
export function useAIChat(noteId?: string, chatScopeId?: string | null, selectedChatId?: string | null) {
    ...
    const send = useCallback(
        async (
            ...
            if (!adapter) {
                ...
                setMessages((prev) => [...removeEmptyAssistantPlaceholder(prev, assistantMessageId), errMsg]);
            ...
            try {
                ...
                if (adapterId !== "cli") {
                    const errMsg = createSystemMessage(chat.id, "Error: Not authenticated. Connect in Settings.");
                    setMessages((prev) => [...removeEmptyAssistantPlaceholder(prev, assistantMessageId), errMsg]);
                    await repositories.ai.saveMessage(errMsg);
                    setIsStreaming(false);
                    return true;
            ...
            try {
                    ...
                    if (event.type === "text") {
                        ...
                        if (cancelledRef.current) {
                            if (!assistantContent.trim()) {
                                setMessages((prev) => removeEmptyAssistantPlaceholder(prev, assistantMessageId));
                                return true;
                            }
                            ...
                            setMessages((prev) =>
                                prev.map((message) => (message.id === assistantMessageId ? partialAssistant : message)),
                            );
                        ...
                        const assistantToSave: AIMessage = {
                            ...assistantMsg,
                            content: assistantContent.trim() ? assistantContent : "No assistant response was returned.",
                            updatedAt: new Date().toISOString(),
                        };
                        setMessages((prev) =>
                            prev.map((message) => (message.id === assistantMessageId ? assistantToSave : message)),
                        );
                        ...
                        setIsStreaming(false);
                        const errMsg = createSystemMessage(chat.id, `Error: ${event.message}`);
                        setMessages((prev) => [...removeEmptyAssistantPlaceholder(prev, assistantMessageId), errMsg]);
                        await repositories.ai.saveMessage(errMsg);
                        return true;
                    }
                ...
                setMessages((prev) =>
                    prev.map((message) => (message.id === assistantMessageId ? fallbackMessage : message)),
                );
                ...
                setIsStreaming(false);
                const errMsg = createSystemMessage(chat?.id || "", `Error: ${getErrorMessage(err)}`);
                setMessages((prev) => [...removeEmptyAssistantPlaceholder(prev, assistantMessageId), errMsg]);
    ...
    const cancel = useCallback(() => {
        ...
        abortControllerRef.current = null;
        setIsStreaming(false);
        const systemMsg = createSystemMessage(chat?.id || "", "Generation stopped.");
