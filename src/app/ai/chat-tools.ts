...
export function createWorkspaceTools() {
  return {
    listNotes: tool({
      description:
        "List workspace notes with lightweight metadata and optional filters.",
            ...
            result: {
              note: {
                ...
                content: includeContent
                  ? truncate(note.content || "", SNIPPET_LENGTH * 3)
                  : undefined,
    ...
    searchNotes: tool({
      description:
        "Search notes by title, tags, or content. Returns concise metadata plus optional excerpt.",
      inputSchema: z.object({
        ...
        try {
          ...
          if (parsed.joins.some((join) => join.table !== "notes")) {
            return {
              success: false,
              message:
                "Joins are currently supported only for the notes dataset and notes aliases.",
            };
          }
