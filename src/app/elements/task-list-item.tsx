...
export const formatTaskFinishedAt = (date = new Date()): string =>
	`${Dates.yearMonthDay(date)} ${Dates.time(date)}`;
...
const getFinishedAtRange = (doc: ProseMirrorNode, taskPos: number) => {
	...
	paragraph.node.descendants((node, pos) => {
		...
		const hasSubscriptMark = node.marks.some(
...
export function setTaskItemChecked(
	...
	const prefix =
		paragraphText.length > 0 && !paragraphText.match(/\s$/) ? " " : "";
				...
				this.editor
					...
					.command(({ tr }) => {
						...
						return position === null
							? false
							: setTaskItemChecked(tr, { taskPos: position, checked });
					})
					.run();
				return true;
			},
