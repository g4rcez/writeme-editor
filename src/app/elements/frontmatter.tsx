...
const FrontmatterView = (props: any) => {
    ...
    useEffect(() => {
        try {
            ...
            if (!note) return;
            if (parsed.title && typeof parsed.title === "string" && parsed.title !== note.title) {
                globalDispatch.syncNoteState({ ...note, title: parsed.title } as any);
            }
            if (parsed.tags) {
                let newTags: string[] = [];
                if (Array.isArray(parsed.tags)) {
                    newTags = parsed.tags.map((t: any) => String(t).trim()).filter((t: string) => t.length > 0);
            ...
            .command(({ tr, dispatch }: any) => {
                if (dispatch) {
                    const paragraphs = lines.map((line: string) =>
                        editor.schema.nodes.paragraph.create({}, line ? editor.schema.text(line) : undefined),
                    );
                ...
                header={
                    <div className="flex justify-between items-center w-full border-t border-card-border">
                        <div className="flex gap-2 items-center py-2 px-4 text-xs text-muted-foreground">
                            ...
                            {error && <span className="ml-1 font-bold text-destructive">!</span>}
                        </div>
