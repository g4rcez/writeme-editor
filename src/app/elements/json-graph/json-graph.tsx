...
const updateDeep = (obj: any, currentPath: string, targetPath: string, val: any, newValue: any): any => {
    if (currentPath === targetPath || (currentPath === "" && targetPath === ROOT_NAME)) {
        try {
            if (newValue === "true") return true;
            if (newValue === "false") return false;
            if (!isNaN(Number(newValue)) && newValue.trim() !== "") return Number(newValue);
    ...
    if (Array.isArray(obj)) {
        return obj.map((item, i) => updateDeep(item, `${currentPath}[${i}]`, targetPath, val, newValue));
    } else if (typeof obj === "object" && obj !== null) {
        const newObj = { ...obj };
        for (const key in newObj) {
            const nextPath = currentPath ? `${currentPath}.${key}` : key;
            newObj[key] = updateDeep(newObj[key], nextPath, targetPath, val, newValue);
        }
        return newObj;
    }
    return obj;
...
const JsonGraphInner = ({ json, onChange }: { json: any; onChange?: (newJson: any) => void }) => {
    const [expandedPaths, setExpandedPaths] = useState<Set<string>>(() => getDefaultExpandedPaths(json));
    ...
    const { nodes: initialNodes, edges: initialEdges } = useMemo(
        () =>
            transformJsonToGraph(json, expandedPaths, onToggle, onValueChange, searchQuery, direction, colors.primary),
        [json, expandedPaths, onToggle, onValueChange, searchQuery, direction, colors.primary],
    );
    ...
    return (
        <div
            className={css(
                ...
                isZenMode ? "fixed inset-0 z-50 bg-background" : "relative bg-background",
            )}
        >
            <div className="flex gap-4 justify-between items-center border-b bg-background border-border/5">
                ...
                <div className="flex gap-1.5 items-center">
                    <Tooltip
                        title={
                            <Button size="small" theme={view === "text" ? "primary" : "muted"} onClick={toggleView}>
                    ...
                    <Tooltip
                        title={
                            <Button size="small" theme="muted" onClick={() => onExport("png")}>
            ...
            >
                {view === "graph" ? (
                    ...
                    <JsonInspectorPanel value={textValue} onChange={onTextChange} className="w-full h-full" />
                )}
            </div>
        </div>
    );
...
export const JsonGraph = (props: { json: any; onChange?: (newJson: any) => void }) => (
    <ReactFlowProvider>
