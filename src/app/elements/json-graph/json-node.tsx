...
export const JsonNode = ({ data }: any) => {
    const { label, value, type, isExpanded, onToggle, matchesSearch, isPathToMatch, onValueChange } = data;
    const [isEditing, setIsEditing] = useState(false);
    const [editValue, setEditValue] = useState(String(value));

    const Icon = type === "object" ? CubeIcon : type === "array" ? ListBulletsIcon : HashIcon;
    ...
    return (
        ...
        >
            <Handle type="target" position={Position.Left} className="!size-2 !bg-primary !border-none" />
            <div className="flex flex-col gap-1.5">
                ...
                {(type === "object" || type === "array") && (
                    <div className="flex justify-between items-center px-0.5">
                        <span className="font-medium tracking-wide text-xs text-muted-foreground">{type}</span>
                        <span className="font-bold rounded text-xs text-muted-foreground">
                            {type === "object" ? Object.keys(data.raw || {}).length : data.raw?.length || 0} items
                        </span>
                    </div>
                )}
            </div>
            <Handle type="source" position={Position.Right} className="!size-2 !bg-primary !border-none" />
        </div>
    );
};
