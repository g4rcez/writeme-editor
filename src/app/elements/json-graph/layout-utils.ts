...
export const getDefaultExpandedPaths = (data: unknown, maxDepth = 2): Set<string> => {
...
export const transformJsonToGraph = (
    ...
    const findMatches = (data: any, path: string, label: string) => {
        ...
        const matches =
            ...
            (label.toLowerCase().includes(searchQuery.toLowerCase()) ||
                (type === "value" && String(data).toLowerCase().includes(searchQuery.toLowerCase())));
        ...
        if (type === "object") {
            ...
            (data as any[]).forEach((value, index) => {
                if (findMatches(value, `${id}[${index}]`, `[${index}]`)) childMatched = true;
            });
        }
    ...
    const traverse = (data: any, path: string, label: string, depth: number, parentId?: string) => {
        const id = path || "$";
        const type = Array.isArray(data) ? "array" : typeof data === "object" && data !== null ? "object" : "value";
        ...
        if (parentId) {
            ...
            edges.push({
                ...
                style: isEdgeHighlighted ? { stroke: highlightColor, strokeWidth: 2 } : undefined,
                markerEnd: {
        ...
        if ((isExpanded || (searchQuery && isPathToMatch)) && (type === "object" || type === "array")) {
            if (type === "object") {
                Object.entries(data).forEach(([key, value]) => {
