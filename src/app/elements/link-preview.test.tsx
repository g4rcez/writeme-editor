...
import { createNoteLinkPreview, createPlainTextExcerpt, findMentionNote } from "./link-preview";
...
describe("link-preview helpers", () => {
    ...
    it("resolves mention previews from label and path fallbacks", () => {
        ...
        expect(
            createNoteLinkPreview([note], { id: "missing-id", label: "daily log", path: "" }, "daily log"),
    ...
    it("creates a plaintext excerpt from markdown content", () => {
        expect(createPlainTextExcerpt("# Heading\nRead [the docs](https://example.com) and `ship it` today.", 34)).toBe(
            "Heading Read the docs and ship it…",
        );
    });

    it("returns a missing note preview when lookup fails", () => {
        expect(
            createNoteLinkPreview([], { id: "missing-id", label: "Missing label", path: "" }, "Missing label"),
        ).toEqual({
            title: "Missing label",
