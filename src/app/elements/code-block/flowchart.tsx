...
const getFlowchartOptions = (isDark: boolean) => {
    ...
    const colors = {
        ...
        successText: parseHslaToHex(theme.colors.success.foreground),
        danger: parseHslaToHex(theme.colors.danger.DEFAULT),
        dangerText: parseHslaToHex(theme.colors.danger.foreground),
        warn: parseHslaToHex(theme.colors.warn.DEFAULT),
        warnText: parseHslaToHex(theme.colors.warn.foreground),
    };
    ...
    return {
        ...
        flowstate: {
            past: { fill: colors.border, "font-color": colors.foreground },
            current: {
                fill: colors.primary,
            ...
            request: { fill: colors.warn, "font-color": colors.warnText },
            invalid: { fill: colors.border, "font-color": colors.foreground },
            approved: {
                fill: colors.success,
                "font-size": 12,
                "yes-text": "APPROVED",
                "no-text": "n/a",
                "font-color": colors.successText,
            },
            rejected: {
                fill: colors.danger,
                "font-size": 12,
...
export const Flowchart = ({ code }: { code: string }) => {
    ...
    if (error) {
        return (
            <div className="p-2 font-mono text-sm text-danger bg-danger-subtle rounded border border-danger/40">
                {error}
            </div>
        );
    }
    ...
    return (
        <div className="relative group overscroll-contain flowchart-block">
            <div className="absolute top-2 right-2 z-10 flex gap-1 rounded-md border border-border bg-background p-1 opacity-0 shadow-soft transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
                <Tooltip
                    title={
                        <Button
                            size="small"
            ...
            {showKeyboardHelp && isZoomEnabled ? (
                <div className="absolute bottom-2 left-2 z-10 rounded-md border border-border bg-background p-2 text-xs text-muted-foreground shadow-soft">
                    <div>+ / - zoom</div>
                    <div>0 reset</div>
            ...
            <div
                ...
                className={css(
                    ...
                    isZoomEnabled && "h-[400px] cursor-move border-border bg-card-background/30",
                )}
