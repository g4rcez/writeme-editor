...
const MATH_CODE_MIRROR_LANGUAGE = StreamLanguage.define<{
  ...
  token(stream, state) {
    ...
    if (
      stream.match(
        /(?:0b[01]+|0o[0-7]+|0x[\da-fA-F]+|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?[ij]?)/,
      )
    ) {
      return "number";
    }
    ...
    if (
      stream.match(
        /[αβγδεζηθικλμνξοπρσςτυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ]|[a-zA-Z]\w*(?:_\w+)?/,
      )
    ) {
      return "variable";
    }
...
function createKeywordCompletionSource(language: string): CompletionSource {
  ...
  const languageCompletions = completionLanguage
    ? LANGUAGE_COMPLETIONS[completionLanguage]
    : [];
...
export const CodeMirrorNodeCodeEditor = ({
  ...
  useEffect(() => {
    ...
    return () =>
      container.removeEventListener("pointerdown", focusEditor, true);
  }, []);
