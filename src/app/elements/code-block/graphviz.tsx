...
const injectTheme = (dot: string, isDark: boolean) => {
  ...
  `;
    ...
    if (firstBrace !== -1) {
        return dot.slice(0, firstBrace + 1) + themeAttributes + dot.slice(firstBrace + 1);
    }
    return dot;
};
        ...
        <div className="relative group overscroll-contain graphviz-block">
            <AnimatePresence>
                {!loading && (
                    <motion.div
                        ...
                        className="flex absolute top-2 overscroll-contain right-2 z-10 gap-1 p-1 opacity-0 rounded-md border shadow-soft transition-opacity bg-background border-border group-hover:opacity-100"
                    >
                        <Tooltip
                            title={
                                <Button
            ...
            >
                <motion.div
                    ...
                    className={css("flex justify-center items-center w-full h-full", isZoomEnabled ? "p-8" : "p-0")}
                >
