...
import { fetchExchangeRates, getCachedRates, setCachedRates, type ExchangeRateData } from "@/lib/currency";

const ResultCell = ({ result, prefix = "", bold }: { result: EvaluatedResult; prefix?: string; bold?: boolean }) => {
    if (!result.ok) {
...
const CommentLine = ({ text }: { text: string }) => (
    <li className="list-none items-center font-mono italic opacity-50 text-muted-foreground">{text}</li>
);

const VariableLine = ({ name, result }: { name: string; result: EvaluatedResult }) => (
...
const LabelLine = ({ label, result }: { label: string; result: EvaluatedResult }) => (
...
const BareLine = ({ raw, result }: { raw: string; result: EvaluatedResult }) => (
    <li className="flex items-center gap-2 font-mono list-none">
        <span>{raw}</span>
...
const TotalLine = ({ kind, result }: { kind: "sum" | "avg"; result: EvaluatedResult }) => (
...
export const MathBlock = (props: { code: string }) => {
    const [ratesData, setRatesData] = useState<ExchangeRateData | null>(() => getCachedRates("EUR")?.data ?? null);
    ...
    const evaluated = useMemo(() => evaluateCode(props.code, ratesData), [props.code, ratesData]);
            ...
            <ul className="list-none list-outside mx-0 flex flex-col gap-0">
                {evaluated.map((line, i) => {
                    switch (line.kind) {
                        ...
                        case "variable":
                            return <VariableLine key={i} name={line.name} result={line.result} />;
                        case "label":
                            return <LabelLine key={i} label={line.label} result={line.result} />;
                        case "bare":
                            return <BareLine key={i} raw={line.raw} result={line.result} />;
                        case "sum":
                        case "avg":
                            return <TotalLine key={i} kind={line.kind} result={line.result} />;
                    }
                })}
            </ul>
        </div>
