...
import { ArrowLeftIcon, CodeIcon, FunnelIcon } from "@phosphor-icons/react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import type { View, ViewColumn } from "@/store/repositories/entities/view";
...
import { astToFilterGroup, filterGroupToQueryString, type FilterGroup } from "@/lib/views/query-builder";
...
export default function ViewDetailPage() {
    ...
    useEffect(() => {
        ...
        repositories.views.getOne(viewId).then((v) => {
            ...
            setTitle(v.title);
            setQuery(v.query);
            setColumns(v.columns.length > 0 ? v.columns : [{ field: "title", label: "Title" }]);
        });
    }, [viewId]);
    ...
    if (!view) {
        return <div className="flex items-center justify-center min-h-full text-foreground text-sm">Loading…</div>;
    }
            ...
            <div>
                ...
                <div className="py-4">
                    {activeTab === "sql" ? (
                        ...
                        <FilterBuilder group={filterGroup} onChange={handleFilterGroupChange} />
                    )}
                </div>
            </div>
