			...
			>
				<div className="flex min-w-0 gap-3">
					...
					<span className="min-w-0 flex-1 space-y-2">
						...
						<span className="block max-h-12 overflow-hidden text-sm leading-6 text-muted-foreground">
							{preview}
						</span>
		...
		<span className="flex flex-wrap gap-1.5">
			{visibleVariables.map((variable) => (
				<span
					...
					title={
						variable.count === 1 ? "Used once" : `Used ${variable.count} times`
					}
				>
					{`{{${variable.name}}}`}
				</span>
	...
	return (
		<div className="space-y-0 p-4" aria-label="Loading templates">
			{Array.from({ length: 4 }, (_, index) => (
				<div
					key={index}
					className="flex gap-3 border-b border-border/20 py-4 last:border-b-0"
				>
