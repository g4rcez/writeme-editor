...
function getReadonlyShortcuts(shortcuts: Shortcut[]): ReadonlyShortcut[] {
    ...
    const appShortcuts = shortcuts
        ...
        .map(({ bind, description }) => ({
            ...
            title: description,
            description:
                WRITEME_SHORTCUT_DESCRIPTIONS[description] ?? `Run the ${description.toLocaleLowerCase()} shortcut.`,
        }));
...
function ReadonlyShortcutList({ shortcuts }: { shortcuts: ReadonlyShortcut[] }) {
    return (
        <section className="mt-8">
            <div className="mb-2 max-w-2xl">
                <h2 className="text-base font-semibold text-foreground">Available shortcuts</h2>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                    These built-in shortcuts are available in the app and are shown here for reference only.
                </p>
            </div>
            <ul>
                {shortcuts.map((shortcut) => (
                    <ReadonlyShortcutRow key={`${shortcut.bind}-${shortcut.title}`} shortcut={shortcut} />
                ))}
            </ul>
        </section>
    );
...
export default function SettingsShortcutsPage() {
    ...
    const readonlyShortcuts = useMemo(() => getReadonlyShortcuts(writemeShortcuts), [writemeShortcuts]);
