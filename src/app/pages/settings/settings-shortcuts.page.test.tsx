...
describe("SettingsShortcutsPage", () => {
	it("shows available app shortcuts as a readonly reference list", () => {
		...
		expect(
			screen.getByText("Switch directly to one of the first eight open tabs."),
		).toBeInTheDocument();
		expect(
			screen.getByText("Switch directly to the last open tab."),
		).toBeInTheDocument();
		...
		expect(
			screen.queryByText("Read-only built-in shortcut."),
		).not.toBeInTheDocument();
		expect(
			screen.queryByText("Start copy watcher mode"),
		).not.toBeInTheDocument();
	});
});
