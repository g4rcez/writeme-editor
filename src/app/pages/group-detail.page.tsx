...
function SortableMemberRow({ member, note, onRemove, onOrderChange }: SortableMemberRowProps) {
...
export default function GroupDetailPage() {
    ...
    useEffect(() => {
        const members = state.noteGroupMembers.filter((m) => m.groupId === groupId).sort((a, b) => a.order - b.order);
    ...
    const handleMerge = async () => {
        ...
        try {
            ...
            for (const member of localMembers) {
                const note = noteMap.get(member.noteId) || (await repositories.notes.getOne(member.noteId));
                if (!note) continue;
                parts.push(`## ${note.title || "Untitled"}\n\n${note.content || ""}`);
            }
            const content = parts.join("\n\n---\n\n");
            const mergedNote = Note.new(`${group?.title ?? "Group"} (merged)`, content);
    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (addNoteContainerRef.current && !addNoteContainerRef.current.contains(e.target as Node)) {
                setAddNoteOpen(false);
                setAddNoteQuery("");
            }
        };
        ...
        <div className="relative flex-col py-6 mx-auto min-h-full max-w-safe">
            ...
            <div className="mb-6">
                ...
                {editingDesc !== null ? (
                    ...
                    >
                        {group.description || <span className="italic opacity-50">Add a description...</span>}
                    </p>
                )}
            </div>
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                    <Button size="small" theme="neutral" onClick={() => setAddNoteOpen((v) => !v)}>
            ...
            {localMembers.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-foreground/40">
                    <p className="text-sm">No notes in this group yet. Add notes from the Notes list.</p>
                </div>
            ) : (
