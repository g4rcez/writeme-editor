import {
  AI_CHAT_LOADING_MESSAGES,
  AIChatMessageList,
} from "@/app/ai/ai-message-item";
...
import {
  ...
  getWorkspaceChatScope,
} from "@/app/ai/workspace-context";
...
export default function ChatPage() {
  ...
  const {
    ...
    clearChat,
    config,
  } = useAIChat(undefined, chatScopeId, selectedChatId);

  const latestMessageContent = messages.at(-1)?.content ?? "";
  const workspaceLabel = state.directory ?? "Local workspace";
  const modelLabel = config
    ? [config.name, config.model || config.adapterId]
        .filter(Boolean)
        .join(" · ")
    : "AI not configured";
  ...
  const submitPrompt = async (value: string): Promise<boolean> => {
    const prompt = value.trim();
    if (!prompt || !config || isLoading || isStreaming) return false;
    return send(
      prompt,
  ...
  const createNewChat = async (): Promise<void> => {
    ...
    const freshChat = await newChat();
    if (freshChat) {
      await dispatch.addAiChatTab(freshChat.id);
      setSearchParams({ chatId: freshChat.id });
    }
  };
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col max-w-safe container mx-auto">
      <div
        ref={listRef}
        ...
        {isLoading ? (
          ...
          <div className="mx-auto flex h-full min-h-64 max-w-2xl flex-col items-center justify-center text-center">
            ...
            <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">
              Ask about notes, trends, drafts, or decisions in this workspace.
              Responses stay as readable markdown.
            </p>
            ...
            <div className="mt-8 grid w-full gap-2 text-left sm:grid-cols-3">
              {PROMPT_EXAMPLES.map((prompt) => (
                <button
                  ...
                  className="rounded-2xl border border-card-border bg-secondary-background p-4 text-left text-sm leading-5 text-foreground transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {prompt}
                </button>
              ))}
      ...
      <div className="shrink-0 bg-background px-4 pb-4 pt-6 md:px-6">
        <div className="mx-auto w-full max-w-safe">
          {isStreaming ? (
            <div className="mb-2 flex items-center justify-between rounded-xl border border-card-border bg-secondary-background px-3 py-2 text-sm text-muted-foreground">
