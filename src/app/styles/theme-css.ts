...
const derivedComponentColors = (colors: TokenTree): TokenTree => {
	...
	const floatingBackground = get(
		colors,
		["floating", "background"],
		background,
	);
	const floatingForeground = get(
		colors,
		["floating", "foreground"],
		foreground,
	);
	...
	const softState = (name: string) => ({
		background: get(
			colors,
			[name, "subtle"],
			alpha(get(colors, [name, "DEFAULT"]), 0.14),
		),
	...
	return {
		...
		button: {
			...
			"ghost-info-background-hover": alpha(
				get(colors, ["info", "DEFAULT"]),
				0.2,
			),
			"ghost-warn-background-hover": alpha(
				get(colors, ["warn", "DEFAULT"]),
				0.2,
			),
			"ghost-danger-background-hover": alpha(
				get(colors, ["danger", "DEFAULT"]),
				0.2,
			),
			"ghost-primary-background-hover": alpha(primary, 0.2),
			"ghost-success-background-hover": alpha(
				get(colors, ["success", "DEFAULT"]),
				0.2,
			),
		...
		modal: {
			"overlay-background": alpha(
				get(colors, ["floating", "overlay"], "hsla(0, 0%, 0%)"),
				0.8,
			),
		},
		radiobox: {
...
const toTuple = (color: string): string =>
	color.match(/^hsla?\((.*)\)$/)?.[1] ?? color;
...
export const createWritemeThemeCss = (
	...
	return [
		createThemeCss(themed, { selector, base }),
		createLegacyCss(selector, themed, base),
	].join("\n\n");
};
