...
describe("Commander", () => {
    ...
    it("opens the create note dialog for the new excalidraw command", () => {
        ...
        const command = getFlattenedCommands().find((item) => item.title === "New excalidraw");
    ...
    it("creates a new terminal tab from the terminal command", async () => {
        ...
        const command = getFlattenedCommands().find((item) => item.title === "New Terminal");
        expect(command).toBeDefined();
        ...
        expect(setOpen).toHaveBeenCalledWith(false);
        expect(mocks.dispatch.addTerminalTab).toHaveBeenCalledWith("terminal-session-id");
        expect(mocks.navigate).toHaveBeenCalledWith("/terminal/terminal-session-id");
    });
    ...
    it("confirms each terminal by name when closing all tabs", async () => {
        ...
        const command = getFlattenedCommands().find((item) => item.title === "Close all tabs");
        expect(command).toBeDefined();
        ...
        const firstConfirm = vi.mocked(uiDispatch.setConfirm).mock.calls.at(-1)?.[0] as ConfirmState;
        ...
        expect(uiDispatch.setConfirm).toHaveBeenLastCalledWith(
            expect.objectContaining({
                ...
                message: 'Closing terminal "Deploy shell" will kill its running shell session.',
            }),
        );

        const secondConfirm = vi.mocked(uiDispatch.setConfirm).mock.calls.at(-1)?.[0] as ConfirmState;
    ...
    it("opens the shortcuts page from the help command without showing the old shortcut binding", () => {
        ...
        const shortcutHelpCommands = getFlattenedCommands().filter((item) => item.title === "Shortcut/Help menu");
