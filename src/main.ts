...
function registerAIHandlers() {
    ...
    ipcMain.on("ai:query", async (event, { commandTemplate, prompt, selection, context, systemPrompt }) => {
        console.log("AI Query received", { prompt });
        AIRunner.run(commandTemplate, { prompt, selection, context, systemPrompt }, event.sender);
    });
    ...
    ipcMain.handle("ai:get-chats", (_, noteId) => {
        try {
            const db = dbManager();
            const stmt = noteId
                ? db.db.prepare("SELECT * FROM aiChats WHERE noteId = ? ORDER BY createdAt DESC")
    ...
    ipcMain.handle("ai:get-messages", (_, chatId) => {
        try {
            const db = dbManager();
            const stmt = db.db.prepare("SELECT * FROM aiMessages WHERE chatId = ? ORDER BY createdAt ASC");
    ...
    ipcMain.handle("ai:clear-messages", (_, chatId: string) => {
        try {
            dbManager().db.prepare("DELETE FROM aiMessages WHERE chatId = ?").run(chatId);
            return { success: true };
        } catch (e: any) {
...
async function checkLinuxUpdate() {
    try {
        const response = await net.fetch("https://api.github.com/repos/g4rcez/writeme-editor/releases/latest");
...
function createNativeAppIcon(size?: number): NativeImage {
    ...
    try {
        const svg = fs.readFileSync(getLogoSvgPath(), "utf8").replace(/var\(--foreground\)/g, "#000000");
...
async function installCliFromMenu(): Promise<void> {
    try {
        ...
        dialog.showErrorBox("Failed to install CLI", error instanceof Error ? error.message : String(error));
    }
}
...
function createOsMenuTemplate(preloadPath: string): MenuItemConstructorOptions[] {
    return [
        {
            label: "Show Writeme",
...
function parseCliArgs(argv: string[], workingDir?: string): { filePath: string | null } {
    const args = getUserArgs(argv);
    for (let index = 0; index < args.length; index++) {
        const arg = args[index];
        if (!arg) continue;
...
function configureWorkspaceSessionData(workspacePath: string | null, isWorkspaceInstance: boolean): void {
    if (!workspacePath || !isWorkspaceInstance) return;

    const workspaceHash = createHash("sha256").update(path.resolve(workspacePath)).digest("hex").slice(0, 16);
    const sessionDataPath = path.join(app.getPath("userData"), "workspace-sessions", workspaceHash);
...
function spawnWorkspaceInstance(folderPath: string): void {
    const child = spawn(process.execPath, getAppRelaunchArgs([WORKSPACE_INSTANCE_FLAG, WORKSPACE_ARG, folderPath]), {
        detached: true,
        env: {
            ...process.env,
...
function sendOpenFile(filePath: string, wait: boolean, requestId: string): void {
...
async function main() {
    ...
    app.on("second-instance", (_, argv, workingDirectory) => {
        ...
        if (workspacePath) {
            if (launchWorkspacePath && path.resolve(launchWorkspacePath) === path.resolve(workspacePath)) {
                showMainWindow();
            } else {
    ...
    app.on("ready", async () => {
        ...
        ipcMain.handle("app:open-folder", (_, { folderPath }: { folderPath: string }) => {
        ...
        ipcMain.handle("app:update-shortcut", (_, newShortcut: string) => {
            ...
            const success = globalShortcut.register(newShortcut, () => createQuickNoteWindow(preload));
            if (!success) {
        ...
        ipcMain.handle("app:update-math-shortcut", (_, newShortcut: string) => {
            ...
            globalShortcut.unregister(previous);
            const success = globalShortcut.register(newShortcut, () => createMathNoteWindow(preload));
            if (!success) {
        ...
        const createTray = () => {
            ...
            tray.setContextMenu(Menu.buildFromTemplate(createOsMenuTemplate(preload)));
            if (process.platform !== "darwin") {
                tray.on("click", () => {
        ...
        if (mainWindow) {
            ...
            const { filePath } = launchWorkspacePath ? { filePath: null } : parseCliArgs(process.argv);
        ...
        if (mainWindow && !isWorkspaceInstance) {
            try {
                ...
                const savedDirectory = dirSetting?.value ? JSON.parse(dirSetting.value) : null;
