...
function replaceVariables(
    ...
    return {
        command: command.replace(/{{prompt}}/g, variables.prompt).replace(/{{system_prompt}}/g, variables.systemPrompt),
...
export class AIRunner {
    ...
    public static async run(
        ...
        const { command, hasSelectionPipe, hasContextPipe } = replaceVariables(commandTemplate, variables);
        ...
        const cleanCommand = tokens
            .map((token) => token.replace("{{selection}}", "").replace("{{context}}", ""))
            .filter(Boolean);
