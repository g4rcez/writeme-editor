...
export function startCliServer(mainWindow: BrowserWindow, options: CliServerOptions = {}): void {
...
export function markCliRendererReady(webContents: WebContents, workspacePath: string | null): void {
...
export function sendToRendererWhenReady(
    ...
    if (!readyWebContentsIds.has(webContentsId) || targetWindow.webContents.isLoading()) {
...
function handleOpenFolderMessage(socket: net.Socket | null, folderPath: string): void {
    if (!windowRef) {
    ...
    const currentWorkspacePath =
        workspacePathByWebContentsId.get(webContentsId) ?? cliServerOptions.getCurrentWorkspacePath?.() ?? null;

    if (currentWorkspacePath && path.resolve(currentWorkspacePath) === path.resolve(folderPath)) {
        windowRef.show();
        windowRef.focus();
