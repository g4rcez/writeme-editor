...
import { defaultCliInstallPath, installBundledCli, resolveBundledCliPath } from "./cli-installer";
...
describe("cli-installer", () => {
    ...
    it("uses a per-user install path on POSIX", () => {
        expect(defaultCliInstallPath({ platform: "darwin", homeDir: "/Users/me" })).toBe(
            "/Users/me/.local/bin/writeme",
        );
    });
    ...
    it("refuses to overwrite a non-symlink POSIX target", async () => {
        ...
        await expect(installBundledCli({ appPath, platform: "linux", installPath })).rejects.toThrow(
    ...
    it("writes a Windows command shim", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "writeme-cli-install-"));
        const appPath = path.join(dir, "app");
        const sourcePath = path.join(appPath, "packages", "cli", "dist", "writeme.exe");
        ...
        await expect(readFile(installPath, "utf8")).resolves.toContain(`"${sourcePath}" %*`);
    });
    ...
    it("replaces an existing symlink", async () => {
        ...
        await expect(installBundledCli({ appPath, platform: "darwin", installPath })).resolves.toMatchObject({
            installPath,
            sourcePath,
        });
    });
