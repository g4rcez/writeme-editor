...
export function resolveBundledCliPath(
  ...
  const relativeCliPath = path.join(
    ...
    "cli",
    "dist",
    cliExecutableName(platform),
  );
  const unpackedAppPath = appPath.replace(
    /([/\\])app\.asar([/\\]|$)/,
...
async function ensureExecutable(
  ...
  try {
    ...
    throw new Error(
      `Bundled CLI not found at ${sourcePath}. Run npm run build:cli before packaging the app.`,
    );
  }
  if (platform !== "win32") {
...
export async function installBundledCli(
  ...
  if (!canInstall) {
    throw new Error(
      `Refusing to overwrite non-symlink file at ${installPath}. Remove it and try again.`,
    );
  }
