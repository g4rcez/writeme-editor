...
export const buildCommitArgs = (dir: string, message: string): string[] => ["-C", dir, "commit", "-m", message];

export const buildPushUpstreamArgs = (dir: string, branch: string): string[] => [
    "-C",
    dir,
    "push",
    "-u",
...
export const isNoUpstreamError = (stderr: string): boolean => /has no upstream branch/i.test(stderr);
...
export const getStatus = async (dir: string, deps: GitDeps = defaultDeps): Promise<GitStatusResult> => {
    const gitBin = await deps.resolveGit();
    if (!gitBin) return { kind: "git-missing" };

    const inside = await deps.runGit(["-C", dir, "rev-parse", "--is-inside-work-tree"], gitBin);
    if (inside.exitCode !== 0) return { kind: "not-a-repo" };

    const status = await deps.runGit(["-C", dir, "status", "--porcelain=v1", "-b"], gitBin);
    if (status.exitCode !== 0) return { kind: "error", stderr: status.stderr || "git status failed" };
 ...
 */
export const getDiff = async (dir: string, deps: GitDeps = defaultDeps): Promise<string> => {
    const gitBin = await deps.resolveGit();
    if (!gitBin) return "";
...
export const commitAndPush = async (
    ...
    const diff = await deps.runGit(["-C", dir, "diff", "--cached", "--quiet"], gitBin);
    ...
    if (push.exitCode === 0) return { kind: "success", pushedRefs: push.stderr || push.stdout };

    if (isNoUpstreamError(push.stderr)) {
        const branchRes = await deps.runGit(["-C", dir, "rev-parse", "--abbrev-ref", "HEAD"], gitBin);
        const branch = branchRes.stdout.trim() || "HEAD";
        const retry = await deps.runGit(buildPushUpstreamArgs(dir, branch), gitBin);
        if (retry.exitCode === 0) return { kind: "success", pushedRefs: retry.stderr || retry.stdout };
        return {
            kind: "error",
            stage: "push",
