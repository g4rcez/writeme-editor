...
export class DatabaseManager {
    ...
    public constructor(dbPath = path.join(app.getPath("userData"), "writeme.sqlite")) {
    ...
    private init() {
        ...
        try {
            const basesTable = this.db
                .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='bases'")
                .get();
            if (basesTable) {
                const viewsTable = this.db
                    .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='views'")
                    .get();
                if (viewsTable) {
                    ...
                    this.db.prepare("INSERT OR IGNORE INTO views SELECT * FROM bases").run();
    ...
    `);
        ...
        const noteColumns = ["url", "description", "favicon", "metadata", "favorite", "deletedAt", "originalFilePath"];
            ...
            try {
                ...
                if (table === "notes") {
                    for (const col of noteColumns) {
                        if (!columns.some((c: any) => c.name === col)) {
                            console.log(`Migrating table ${table}: adding '${col}' column`);
                            const type = col === "favorite" ? "INTEGER DEFAULT 0" : "TEXT DEFAULT NULL";
                ...
                if (table === "aiConfigs") {
                    ...
                    try {
                        this.db.prepare(`UPDATE aiConfigs SET adapterId = 'cli' WHERE adapterId IS NULL`).run();
                    } catch (error) {
                ...
                if (table === "aiCredentials") {
                    for (const col of aiCredentialColumns) {
                        if (!columns.some((c: any) => c.name === col)) {
                            this.db.prepare(`ALTER TABLE aiCredentials ADD COLUMN ${col} TEXT`).run();
                        }
                    }
                }

                if (table === "tabs") {
                    const currentColumns = this.db.prepare(`PRAGMA table_info(${table})`).all() as any[];
                    if (!currentColumns.some((c: any) => c.name === "scrollY")) {
                        console.log(`Migrating table ${table}: adding 'scrollY' column`);
                        this.db.prepare(`ALTER TABLE ${table} ADD COLUMN scrollY REAL DEFAULT 0`).run();
                    }
                }
            } catch (e) {
    ...
    private collection(table: string): DatabaseCollection {
        if (!isDatabaseCollection(table)) throw new TypeError(`Unknown database collection: ${table}`);
        return table;
    }
    ...
    public migrateCollection(table: string, records: unknown[]): MigrationCounts {
        ...
        this.db.transaction(() => {
            for (const record of records) {
                ...
                if (collection === "settings" || collection === "cursorPositions") source.id = String(destination.id);
                const sourceEntries = Object.entries(source).filter(([, value]) => value !== undefined);
                const identical = sourceEntries.every(([key, value]) => {
                    ...
                    return (
                        JSON.stringify(serialized) === JSON.stringify(stored) || JSON.stringify(serialized) === stored
                    );
                });
                if (identical) {
                    counts.identical++;
    ...
    public save(
        ...
        const stmt = this.db.prepare(`INSERT OR REPLACE INTO ${collection} (${columns}) VALUES (${placeholders})`);
        stmt.run(...values);
        return parsed;
    }
    ...
    public delete(table: string, id: string): void {
        ...
        if (GENERIC_DELETE_DENYLIST.has(collection)) {
            throw new TypeError(`Generic deletion is not allowed for collection: ${collection}`);
        }
    ...
    public getRecentNotes(limit: number, workspacePath: string | null): any[] {
        const workspacePrefix = workspacePath ? `${workspacePath.replace(/[\\/]+$/, "")}${path.sep}` : null;
        const stmt = this.db.prepare(
            `SELECT * FROM notes
        ...
        const results = stmt.all(workspacePath, workspacePath, workspacePrefix, workspacePrefix, limit) as any[];
        return results.map((row) => this.normalizeRow(row));
    }
    ...
    public softDeleteNote(id: string, deletedAt: string): void {
        this.db.transaction(() => {
            this.db.prepare("UPDATE notes SET deletedAt = ? WHERE id = ?").run(deletedAt, id);
            this.db.prepare("DELETE FROM tabs WHERE noteId = ? AND (type IS NULL OR type != 'ai-chat-tab')").run(id);
        })();
    }

    public hardDeleteNote(id: string): void {
        this.db.transaction(() => {
            this.db.prepare("DELETE FROM aiMessages WHERE chatId IN (SELECT id FROM aiChats WHERE noteId = ?)").run(id);
    ...
    public getTrashedNotes(): any[] {
        const stmt = this.db.prepare(`SELECT * FROM notes WHERE deletedAt IS NOT NULL ORDER BY deletedAt DESC`);
    ...
    public deleteTabsByNoteId(noteId: string): void {
        const stmt = this.db.prepare("DELETE FROM tabs WHERE noteId = ? AND (type IS NULL OR type != 'ai-chat-tab')");
        stmt.run(noteId);
    }
    ...
    public getNoteGroupMembersByGroupId(groupId: string): any[] {
        const stmt = this.db.prepare(`SELECT * FROM noteGroupMembers WHERE groupId = ? ORDER BY "order" ASC`);
        return (stmt.all(groupId) as any[]).map((row) => this.normalizeRow(row));
    }

    public reorderNoteGroupMembers(members: { id: string; order: number }[]): void {
        const updateStmt = this.db.prepare(`UPDATE noteGroupMembers SET "order" = ? WHERE id = ?`);
    ...
    public deleteNoteGroupMembersByNoteId(noteId: string): void {
        const stmt = this.db.prepare("DELETE FROM noteGroupMembers WHERE noteId = ?");
        stmt.run(noteId);
    }

    public deleteNoteGroupMembersByGroupId(groupId: string): void {
        const stmt = this.db.prepare("DELETE FROM noteGroupMembers WHERE groupId = ?");
        stmt.run(groupId);
    }

    public getNoteByFilePath(filePath: string): any {
        const stmt = this.db.prepare("SELECT * FROM notes WHERE filePath = ? AND deletedAt IS NULL LIMIT 1");
