...
export function parseAiCredentials(
  value: unknown,
  ...
  const unknown = Object.keys(input).filter(
    (key) => !(key in aiCredentialsSchema.shape),
  );
  if (unknown.length) {
    console.warn(
      `Stripped unknown aiCredentials fields: ${unknown.join(", ")}`,
    );
  }
  return parsed;
}
...
export type DatabaseRecord<C extends DatabaseCollection = DatabaseCollection> =
  z.output<(typeof databaseSchemas)[C]> & { id: string };

export function isDatabaseCollection(
  value: string,
): value is DatabaseCollection {
...
export function parseDatabaseRecord<C extends DatabaseCollection>(
  ...
  const input = value as Record<string, unknown>;
  const parsed = databaseSchemas[collection].parse(input) as Record<
    string,
    unknown
  >;
  ...
  const unknown = Object.keys(input).filter(
    (key) => !(key in databaseSchemas[collection].shape),
  );
  if (unknown.length) {
