...
const makeRunGit =
    ...
    async (args) => {
        ...
        return responses[key] ?? { stdout: "", stderr: `unmocked: ${key}`, exitCode: 99 };
    };

const makeDeps = (responses: Partial<Record<string, RunResult>>, gitBin: string | null = "/usr/bin/git"): GitDeps => ({
...
describe("git-runner argv + classifiers", () => {
    ...
    it("push upstream args include -u origin <branch>", () => {
        expect(buildPushUpstreamArgs("/tmp/repo", "main")).toEqual(["-C", "/tmp/repo", "push", "-u", "origin", "main"]);
    });

    it("isNoUpstreamError recognises common stderr variants", () => {
        expect(isNoUpstreamError("fatal: The current branch foo has no upstream branch.\n")).toBe(true);
        expect(isNoUpstreamError("error: failed to push some refs to 'origin'")).toBe(false);
    });
});
...
describe("commitAndPush", () => {
    ...
    it("no-upstream fallback: retries with -u origin <branch> and succeeds", async () => {
        const noUpstreamStderr = "fatal: The current branch feat has no upstream branch.";
        const deps = makeDeps({
            "rev-parse --is-inside-work-tree": ok(),
            "add .": ok(),
            "diff --cached --quiet": fail(),
            ...
            "push -u origin feat": ok("", "branch 'feat' set up to track 'origin/feat'"),
        });
        const result = await commitAndPush(DIR, MSG, deps);
        expect(result).toEqual({
            kind: "success",
