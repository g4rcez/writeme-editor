...
describe("Tab Management Logic", () => {
	beforeEach(async () => {
		...
		await useGlobalStore.dispatchers.init(
			"light" as any,
			[],
			[],
			16,
	...
	it("creates terminal tab metadata and selects the new tab", async () => {
		...
		expect(state.terminalSessions).toMatchObject([
			{ id: "terminal-1", title: "Terminal", project: "workspace-a" },
		]);
	...
	it("renames terminal session metadata", async () => {
		...
		await useGlobalStore.dispatchers.renameTerminalSession(
			"terminal-1",
			"Build logs",
		);
	...
	it("removes terminal metadata when closing a terminal tab", async () => {
		...
		expect(repositories.terminalSessions.delete).toHaveBeenCalledWith(
			"terminal-1",
		);
	});
});
