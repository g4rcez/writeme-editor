...
beforeAll(() => {
  ...
  `).run();
    ...
    db.prepare(`INSERT OR REPLACE INTO hashtags (id, hashtag, filename) VALUES (?, ?, ?)`).run("h1", "work", "/a.md");

    db.prepare(`INSERT OR REPLACE INTO settings (id, name, value, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)`).run(
        "s1",
        "theme",
        '"dark"',
        "2024-01-01",
