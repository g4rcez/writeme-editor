...
import { QueryNotesArgsSchema, QuerySettingsArgsSchema, QueryTagsArgsSchema } from "../schemas/query.ts";

describe("OpenFileArgsSchema", () => {
    test("accepts absolute path", () => {
        expect(() => OpenFileArgsSchema.parse({ filePath: "/some/file.md", wait: false })).not.toThrow();
    });

    test("rejects relative path", () => {
        expect(() => OpenFileArgsSchema.parse({ filePath: "relative/path.md", wait: false })).toThrow();
    });

    test("accepts null filePath", () => {
        expect(() => OpenFileArgsSchema.parse({ filePath: null, wait: false })).not.toThrow();
    });
...
describe("OpenFolderArgsSchema", () => {
    test("accepts absolute folder path", () => {
        expect(() => OpenFolderArgsSchema.parse({ folderPath: "/some/folder" })).not.toThrow();
    });

    test("rejects relative folder path", () => {
        expect(() => OpenFolderArgsSchema.parse({ folderPath: "relative/folder" })).toThrow();
    });
});
