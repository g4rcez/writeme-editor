...
export function queryTags(
  ...
  try {
    if (opts.note) {
      const noteRow = db
        .prepare("SELECT filePath FROM notes WHERE id = ? LIMIT 1")
        .get(opts.note) as { filePath: string | null } | null;
      const filePath = noteRow?.filePath ?? null;
      if (!filePath) return [];
      return db
        .prepare(
          "SELECT * FROM hashtags WHERE filename = ? ORDER BY hashtag ASC",
        )
        .all(filePath) as Hashtag[];
    }
    return db
      .prepare("SELECT * FROM hashtags ORDER BY hashtag ASC")
      .all() as Hashtag[];
  } finally {
    db.close();
  }
...
export function querySettings(dbPath = getDbPath()): Setting[] {
  ...
  try {
    return db
      .prepare("SELECT * FROM settings ORDER BY name ASC")
      .all() as Setting[];
  } finally {
    db.close();
  }
...
export function queryProjects(dbPath = getDbPath()): Project[] {
  ...
  try {
    return db
      .prepare("SELECT * FROM projects ORDER BY title ASC")
      .all() as Project[];
  } finally {
    db.close();
  }
