...
const CURRENCY_HINT_RE =
  /[$€£¥₩₹₽₺฿₱₫]\s*\d|\b\d[\d.,]*\s+[A-Za-z]{3}\s+(?:to|in)\s+[A-Za-z]{3}\b/i;
...
export async function appendMathHistory(
  expression: string,
  historyPath = getMathHistoryPath(),
): Promise<void> {
...
function renderResult(
  result: Extract<EvaluatedLine, { result: unknown }>["result"],
): string {
