import { queryNotes, queryProjects, querySettings, queryTags } from "../lib/db.ts";
import {
...
function printTable<T extends Record<string, unknown>>(rows: T[], columns: (keyof T)[]): void {
    if (rows.length === 0) {
        console.log("No results.");
        return;
    }

    const widths = columns.map((col) => Math.max(String(col).length, ...rows.map((r) => String(r[col] ?? "").length)));
...
export async function handleQuery(subcommand: string, rawArgs: Record<string, unknown>): Promise<void> {
    switch (subcommand) {
        case "notes": {
        ...
        case "projects": {
            ...
            if (args.json) {
                ...
                printTable(rows as Record<string, unknown>[], ["id", "title", "folderPath"]);
            }
            break;
        }
        default: {
